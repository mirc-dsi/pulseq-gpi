...
if isempty(parser)
    ...
    addParamValue(parser,'sliceThickness',0,@isnumeric);
     addParamValue(parser,'grad_dir',0,@ischar);
end
parse(parser,flip,varargin{:});
opt = parser.Results;
grad_dir = opt.grad_dir;
...
if nargout>1
    ...
    %% Slice selection could happen on any axis - SG 
    if(grad_dir == 'x')
            gz = mr.makeTrapezoid('x',opt.system,'flatTime',opt.duration,'flatArea',area);
    elseif (grad_dir == 'y')
            gz = mr.makeTrapezoid('y',opt.system,'flatTime',opt.duration,'flatArea',area);
    else %default z
